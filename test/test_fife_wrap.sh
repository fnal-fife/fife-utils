...
test_parallel() {
    export POMS_TASK_ID=9999
    mkdir /tmp/out$$
    ../libexec/fife_wrap --debug --limit 4 --multifile --appname demo --appfamily demo --appvers v1_0  --exe cat --addoutput '*.root' --rename unique --declare_metadata --add_location --dest=/tmp/out$$ --hash 2 -- '>bar${nthfile}.root' '<' 
    res=$?
    ls -Rl /tmp/out$$
    rm -rf /tmp/out$$
    return $res
